    ...
    methods: {
        getArrFromRes (res, keys) {
            if (res && keys[0] && res.hasOwnProperty(keys[0])) {
                if (!keys[1]) { return res[keys[0]] } else { this.getArrFromRes(res[keys[0]], keys.shift()) }
            } else { return [] }
        },
        ...
        formatterListDisplay (column, command) {
            ...
            if (subKey) { rst = rst + '(' + command[subKey] + ')' }
